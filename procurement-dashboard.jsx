...
function ProQureApp({ session, companyId, memberships, onNeedMfa, onRechoose }) {
  ...
  const [nrStep,   setNrStep]   = useState(1);     // New-request guided sub-step (1..4): jobref -> site -> trade -> materials
  ...
  return (
    <div data-theme={darkMode?"dark":"light"} style={{fontFamily:"'Plus Jakarta Sans','Helvetica Neue',sans-serif",background:"var(--bg-page)",color:"var(--text-primary)",transition:"background 0.3s,color 0.2s"}} className="app-shell">
      ...
      *{box-sizing:border-box;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale}
      ...
      .nr-ring{position:absolute;left:50%;top:38px;width:56px;height:56px;border-radius:50%;border:2px solid var(--green);transform:translate(-50%,-50%) scale(1);opacity:0;z-index:1}
      .nr-mic.live .nr-ring{border-color:var(--red);animation:nrRingPulse 1.6s ease-out infinite}
      @keyframes nrMicPulse{0%,100%{transform:scale(1)}50%{transform:scale(1.08)}}
      @keyframes nrRingPulse{0%{opacity:0.6;transform:translate(-50%,-50%) scale(1)}100%{opacity:0;transform:translate(-50%,-50%) scale(2.4)}}
      .nr-eq{display:none;justify-content:center;align-items:flex-end;gap:4px;height:20px;margin-top:12px;position:relative;z-index:2}
      .nr-mic.live .nr-eq{display:flex}
      ...
      <div key={view} style={{marginLeft:isMobile?0:240,marginRight:(!isMobile&&notifOpen)?380:0,padding:isMobile?"calc(76px + env(safe-area-inset-top)) 16px calc(88px + env(safe-area-inset-bottom))":"32px 40px",animation:"fadeIn 0.28s cubic-bezier(0.16,1,0.3,1)",position:"relative",zIndex:1,transition:"margin-right .28s cubic-bezier(0.16,1,0.3,1)"}} className="main-content">
            ...
            {step===2&&parsed&&(
              <div>
                ...
                <div className="nr-step" key={cfgStep} style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"minmax(0,0.82fr) minmax(0,1.1fr)",gap:isMobile?20:56,alignItems:"start"}}>
                  ...
                  {cfgStep===1&&(
                    <>
                      <div style={{background:"var(--bg-card-solid)",border:"1px solid var(--border)",borderRadius:"var(--radius-lg)",padding:"20px 22px",boxShadow:"var(--shadow-sm)",marginBottom:16}}>
                        <div style={{fontSize:13,fontWeight:600,color:"var(--text-primary)",marginBottom:12}}>{parsed.items?.length||0} item{(parsed.items?.length||0)===1?"":"s"} - all fields editable</div>
                  ...
                  {/* ---- Sub-step 2: Response deadline ---- */}
                  {cfgStep===2&&(
                    <div style={{background:"var(--amber-light)",border:"1px solid var(--amber)",borderRadius:"var(--radius-md)",padding:"18px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
                      <div>
                        <div style={{fontSize:13,fontWeight:700,color:"var(--amber)",marginBottom:2}}>Response deadline (optional)</div>
                        <div style={{fontSize:12,color:"var(--amber)"}}>Ask suppliers to respond before this date</div>
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                  ...
                  {/* ---- Sub-step 3: Delivery requirements ---- */}
                  {cfgStep===3&&(
                    <div style={{background:"var(--indigo-light)",border:"1px solid var(--border)",borderRadius:"var(--radius-md)",padding:"18px 20px"}}>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(4,1fr)",gap:8}}>
                        {[{val:"direct",label:"To site",icon:"truck"},{val:"alternative",label:"Alt. address",icon:"building"},{val:"collect",label:"Collect",icon:"store"},{val:"tbc",label:"TBC",icon:"question"}].map(opt=>(
                          <label key={opt.val} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6,padding:"12px 8px",borderRadius:"var(--radius-sm)",border:`1.5px solid ${deliveryMethod===opt.val?"var(--green-dark)":"var(--border)"}`,background:deliveryMethod===opt.val?"var(--green-mint)":"var(--bg-card-solid)",cursor:"pointer",textAlign:"center"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:12,marginTop:26,flexWrap:"wrap"}}>
                    {(cfgStep===1||cfgStep===4)&&<button onClick={()=>setTemplateModal(true)} style={{fontSize:12,color:"var(--indigo)",background:"var(--indigo-light)",border:"1px solid var(--border)",borderRadius:"var(--radius-sm)",padding:"10px 16px",cursor:"pointer",fontWeight:600}}>Save as template</button>}
                    {cfgStep===2&&<button className="nr-skip" onClick={()=>setCfgStep(3)}>Skip for now</button>}
                    <div style={{flex:1}}/>
                    {cfgStep<4
        ...
        {view==="companies"&&(
          <div className="stagger-in" style={{maxWidth:720}}>
            ...
            <p style={{fontSize:14,color:"var(--text-secondary)",marginBottom:24}}>You're a member of more than one company. Switch between them here - each one is completely separate, and its data is never shared with the others.</p>
            <Card>
              <div style={{fontSize:15,fontWeight:600,color:"var(--text-primary)",marginBottom:4}}>Your companies</div>
              <div style={{fontSize:12.5,color:"var(--text-secondary)",marginBottom:14}}>Switching reloads ProQure with that company's data.</div>
